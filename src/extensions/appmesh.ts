import { CfnMapping, Duration, Stack } from 'aws-cdk-lib';
...
export class AppMeshExtension extends ServiceExtension {
  ...
  public useTaskDefinition(taskDefinition: ecs.TaskDefinition) {
    var region = Stack.of(this.scope).region;
    var partition = Stack.of(this.scope).partition;
    var appMeshRepo;
  ...
  public useService(service: ecs.Ec2Service | ecs.FargateService) {
    ...
    function addListener(protocol: Protocol, port: number): appmesh.VirtualNodeListener {
      switch (protocol) {
        case Protocol.HTTP :
  ...
  public connectToService(otherService: Service, _connectToProps?: ConnectToProps) {
